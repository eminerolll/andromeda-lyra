...
function render() {
  ...
  // User ports
  html +=
    '<div class="section-label" style="margin-bottom:12px;">Aktif Portlar (' +
    portData.user.length +
    ")</div>";

  if (portData.user.length === 0) {
    html +=
      '<div style="text-align:center; padding:30px; color:var(--text-muted); font-size:13px;">Aktif dev server yok</div>';
  } else {
    html +=
      '<table class="ports-table"><thead><tr><th>Port</th><th>Proses</th><th>Proje</th><th>RAM</th><th>Uptime</th><th></th></tr></thead><tbody>';
    ...
    for (const p of portData.user) {
      ...
      html +=
        "<td>" +
        (p.project ? escapeHtml(p.project) : '<span style="color:var(--text-muted)">-</span>') +
        "</td>";
      html += "<td>" + escapeHtml(p.memory || "?") + "</td>";
      html += "<td>" + escapeHtml(p.uptime || "?") + "</td>";
      html += '<td style="text-align:right;">';
      html +=
        '<a href="' + url + '" target="_blank" class="btn btn-sm" style="margin-right:4px;">Ac</a>';
      html +=
        '<button class="btn btn-sm" style="color:var(--red);" data-kill-port="' +
        escapeHtml(p.port) +
        '">Durdur</button>';
      html += "</td></tr>";
    }
  ...
  for (const p of portData.system) {
    ...
    if (codePort && p.port === codePort) {
      html +=
        '<a href="' +
        escapeHtml(serviceUrl("code") + "/") +
        '" target="_blank" class="btn btn-sm">Ac</a>';
    }
    html += "</td></tr>";
  }
  html += "</tbody></table>";

...
async function loadGitData() {
  if (!currentProject) {
    document.getElementById("gitContent").innerHTML =
      '<div style="text-align:center; padding:40px; color:var(--text-muted);">Bir proje sec</div>';
    return;
  }
  ...
  try {
    const [status, log, diff] = await Promise.all([
      ...
      api("/api/git/" + currentProject + "/diff").catch(() => ({
        unstaged: "",
        staged: "",
        files: []
      }))
...
function renderGitDashboard(status, log, diff) {
  ...
  html +=
    '<span style="font-family:var(--mono); font-size:14px; font-weight:600; color:var(--accent);">' +
    escapeHtml(status.branch) +
    "</span>";
  if (status.ahead || status.behind) {
    html += '<span style="font-size:12px; color:var(--text-muted);">';
    if (status.ahead)
      html += '<span style="color:var(--green);">&uarr;' + status.ahead + "</span> ";
    if (status.behind)
      html += '<span style="color:var(--orange);">&darr;' + status.behind + "</span>";
    html += "</span>";
  }
  html += "</div>";
  html += '<div style="display:flex; gap:12px; font-size:12px; color:var(--text-muted);">';
  if (status.staged)
    html += '<span style="color:var(--green);">' + status.staged + " staged</span>";
  if (status.unstaged)
    html += '<span style="color:var(--orange);">' + status.unstaged + " unstaged</span>";
  if (status.untracked)
    html += '<span style="color:var(--text-muted);">' + status.untracked + " untracked</span>";
  html += "</div>";
  if (status.lastCommit) {
    html +=
      '<div style="margin-left:auto; font-size:12px; color:var(--text-muted);">Son: ' +
  ...
  html +=
    '<button class="btn btn-sm" data-git-action="reset-hard" style="color:var(--red);">Reset</button>';
  html += "</div>";
  html += "</div>";
  ...
  if (diff.files.length === 0) {
    html +=
      '<div style="padding:16px; text-align:center; color:var(--text-muted); font-size:12px;">Degisiklik yok</div>';
  } else {
    for (const f of diff.files) {
      const statusColor = f.status.includes("M")
        ? "var(--orange)"
        : f.status.includes("A")
          ? "var(--green)"
          : f.status === "D"
            ...
            : f.status === "?"
              ...
              : "var(--text)";
      html +=
        '<div class="log-entry"><span style="color:' +
        statusColor +
  ...
  if (Array.isArray(log) && log.length) {
    for (const c of log) {
      if (c.hash) {
        ...
        html +=
          '<span class="log-date">' + escapeHtml(c.date ? c.date.split(" ")[0] : "") + "</span>";
        html += "</div>";
      }
    }
  } else {
    html +=
      '<div style="padding:16px; text-align:center; color:var(--text-muted); font-size:12px;">Commit yok</div>';
  }
  html += "</div></div>";

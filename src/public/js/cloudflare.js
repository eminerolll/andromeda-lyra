...
let status = { enabled: false, integrationEnabled: false, active: false, mode: "remote", canManage: false };
let entries = [];
...
async function loadAll() {
  try {
    ...
    if (container) {
      container.innerHTML = '<div style="padding:40px; text-align:center; color:var(--red);">Yuklenemedi: '
...
function updateHealthDots() {
  document.querySelectorAll("[data-health-host]").forEach(el => {
    ...
    if (!h) {
      ...
      el.title = h.code ? h.code + (h.latency ? " (" + h.latency + "ms)" : "") : (h.reason || "bilinmiyor");
    }
  });
}
...
function infoBox(text, color) {
  return '<div style="font-size:12px; color:' + (color || "var(--text-muted)")
...
function renderHeader() {
  ...
  html += '<div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">' + dot + " <span>" + statusText + "</span>";
  html += '<span class="badge" style="font-size:11px; color:var(--text-muted); border:1px solid var(--border); padding:2px 8px; border-radius:10px;">'
    + escapeHtml(MODE_LABEL[status.mode] || status.mode) + "</span>";
  if (settings.tunnelId) {
    html += '<span style="color:var(--text-muted); font-size:11px; font-family:JetBrains Mono, monospace;">'
      + escapeHtml(settings.tunnelId) + "</span>";
  }
  html += "</div>";
...
function renderTokenPanel(alwaysOpen) {
  let html = '<div id="cfSettingsPanel" class="side-card" style="margin-bottom:16px;'
  ...
  if (settings.hasToken) {
    html += '<div style="margin-bottom:12px; font-size:13px;">Token: <span style="font-family:JetBrains Mono, monospace;">'
      + escapeHtml(settings.tokenPreview) + "</span>";
    if (settings.zoneDomain) {
      html += ' &middot; Zone: <span style="color:var(--green, #4ade80);">' + escapeHtml(settings.zoneDomain) + "</span>";
    }
    html += "</div>";
    html += '<button class="btn btn-sm" id="cfTokenClear" style="color:var(--red);">Token\'i Sil</button>';
  } else {
    html += '<form id="cfTokenForm" style="display:flex; gap:8px; flex-wrap:wrap;">';
    html += '<input type="password" class="form-input" id="cfTokenInput" placeholder="Cloudflare API token..." style="flex:1; min-width:220px;" required>';
...
function renderRemote() {
  ...
  html += '<form id="cfDiscoverForm" class="side-card" style="display:flex; flex-wrap:wrap; gap:12px; align-items:end; margin-bottom:12px;">';
  ...
  html += '<input type="text" class="form-input" id="cfDiscoverTunnel" placeholder="otomatik kesfedilir">';
  html += "</div>";
  html += '<div class="form-group" style="margin-bottom:0; flex:1; min-width:180px;">';
  html += '<label class="form-label">Domain</label>';
  html += '<input type="text" class="form-input" id="cfDiscoverDomain" placeholder="'
    + escapeHtml(settings.zoneDomain || "example.com") + '">';
  html += "</div>";
  html += '<button type="submit" class="btn btn-primary"'
    + (settings.hasToken ? "" : " disabled title=\"Once token kaydet\"") + ">Kesfet</button>";
  html += "</form>";
  html += infoBox("Kesif sirasinda tunnel id sunucudaki cloudflared servisinden okunmaya calisilir. "
...
function renderTable() {
  ...
  let html = '<div class="section-label" style="margin-bottom:12px;">Mevcut Ingress (' + rows.length + ")</div>";

  if (!rows.length) {
    html += '<div style="text-align:center; padding:30px; color:var(--text-muted);">Kayit yok</div>';
    return html;
  }

  html += '<table class="ports-table"><thead><tr><th>Hostname</th><th>Servis</th><th>Tip</th><th></th></tr></thead><tbody>';
  for (const e of rows) {
    const hostname = e.hostname || "(catch-all)";
    const tip = e.isCatchAll ? "fallback" : e.isWildcard ? "wildcard" : e.isProtected ? "korumali" : "normal";
    const tipColor = e.isProtected
      ? "var(--yellow, #fbbf24)"
      : e.isCatchAll || e.isWildcard ? "var(--text-muted)" : "var(--green, #4ade80)";
    const pingable = e.hostname && !e.isCatchAll && !e.isWildcard && e.service
    ...
    html += "<td>";
    if (pingable) {
      html += '<span data-health-host="' + escapeHtml(e.hostname)
        + '" style="margin-right:6px; color:var(--text-muted);" title="kontrol ediliyor">●</span>';
    }
    html += escapeHtml(hostname) + "</td>";
    html += '<td style="font-family:JetBrains Mono, monospace; font-size:12px;">' + escapeHtml(e.service || "-") + "</td>";
    html += '<td style="color:' + tipColor + '; font-size:12px;">' + tip + "</td>";
    html += '<td style="text-align:right;">';
    if (e.hostname && !e.isCatchAll) {
      html += '<a href="https://' + encodeURI(e.hostname) + '" target="_blank" class="btn btn-sm" style="margin-right:4px;">Ac</a>';
    }
    if (deletable) {
      html += '<button class="btn btn-sm" data-remove="' + escapeHtml(e.hostname) + '" style="color:var(--red);">Sil</button>';
    }
    html += "</td></tr>";
  }
  html += "</tbody></table>";
...
function renderManaged() {
  ...
  if (!status.integrationEnabled) {
    html += infoBox("Cloudflare entegrasyonu kapali; ingress goruntulenemez. Ayarlar &gt; Entegrasyonlar'dan ac.",
      "var(--yellow, #fbbf24)");
    return html;
  }
  ...
  if (status.mode === "api" && ingressSource && ingressSource !== "cloudflare") {
    html += infoBox("Uyari: Cloudflare bu tunnel'in yapilandirmasini &quot;" + escapeHtml(ingressSource)
  ...
  html += '<form id="cfAddForm" class="side-card" style="display:flex; flex-wrap:wrap; gap:12px; align-items:end; margin-bottom:12px;">';
  html += '<div class="form-group" style="margin-bottom:0; flex:2; min-width:220px;">';
  html += '<label class="form-label">Hostname</label>';
  html += '<input type="text" class="form-input" id="cfHostname" placeholder="app.'
    + escapeHtml(settings.zoneDomain || "example.com") + '" required>';
  html += "</div>";
  html += '<div class="form-group" style="margin-bottom:0; flex:1; min-width:100px;">';
  html += '<label class="form-label">Local Port</label>';
  html += '<input type="number" class="form-input" id="cfPort" placeholder="8090" min="1" max="65535" required>';
  html += "</div>";
  html += '<div class="form-group" style="margin-bottom:0;">';
  html += '<label style="font-size:12px; display:flex; align-items:center; gap:6px; padding-bottom:8px;">';
...
function conflictText(host, conflicts) {
  ...
  return host + " icin zaten DNS kaydi var:\n" + lines.join("\n")
...
function attachHandlers(root) {
  ...
  root.querySelectorAll("[data-remove]").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      const dropDns = settings.hasToken
        && confirm(host + " icin DNS kaydi da silinsin mi?\n\n"
          + "Hayir dersen ingress silinir, DNS kaydi Cloudflare'de kalir.");
      btn.disabled = true;
      const r = await cfFetch("/api/cf/ingress/" + encodeURIComponent(host) + (dropDns ? "?dns=1" : ""),
        { method: "DELETE" });
      btn.disabled = false;
      if (!r.ok) return toast(r.data.error || "Silinemedi", "error");
